      ...
      summary: {
        ...
        overallProgress:
          totalTarget > 0
            ? (totalCurrent / totalTarget) * 100
            : 0,
        completedCount: savings.filter(
          item => item.isCompleted
        ).length,
        inProgressCount: savings.filter(
          item => !item.isCompleted
        ).length,
      },
    });
...
exports.deleteSavings = async (req,res)=>{
  ...
  try {
    ...
    const savings =
      await Savings.findById(req.params.id);
    ...
    await Savings.findByIdAndDelete(
      req.params.id
    );
